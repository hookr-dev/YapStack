...
// ------------------------------------------------------------------ §13.7 recovery split KAT
#[test]
fn kat_13_7_recovery_split() {
    // §13.7 pinned input: recovery_bytes(20) = 000102..13. NOTE: the LITERAL §13.7 output
    // vectors below pin the derivation — an earlier version of this test compared
    // recovery_key(x) against hex::encode(recovery_key(x)) (f(x) == f(x)), an
    // unfalsifiable self-comparison that caught no drift and never touched block 2.
    let recovery: [u8; 20] = hx("000102030405060708090a0b0c0d0e0f10111213")
        .try_into()
        .unwrap();

    // Shared helper pinning BOTH stacks: the same `kdf::recovery_split` the desktop auth
    // ceremony runs in production. Assert both blocks against the LOCKED §13.7 literals.
    let (recovery_key, recovery_auth_key) = kdf::recovery_split(&recovery);
    assert_eq!(
    ...
    assert_eq!(
        hex::encode(recovery_auth_key),
        "1e5ad1dbc106f5791bb28d514097f462fece2837b3b5a02b6708364d6655db8b",
        "recovery_auth_key drifted from the LOCKED §13.7 vector"
    );

    // Block 1 stays byte-identical to the 32-byte `recovery_key` primitive, so the L=64
    // split does not disturb the existing on-disk `wrapped_vault_key_recovery`.
    assert_eq!(recovery_key.to_vec(), kdf::recovery_key(&recovery));
    // Block 2 is domain-separated from the wrap key.
    assert_ne!(recovery_key, recovery_auth_key);

    // The wrap key must actually wrap a vault key round-trip under the committing envelope.
    let vault_key = [0x5au8; 32];
    let nonce = [0x11u8; 24];
    let aad = aead::lp(&[&[VERSION], b"yapstack.wrap.vault.rec.v1"]);
    let blob = aead::seal_committing(&recovery_key, &nonce, &vault_key, &aad).unwrap();
    assert_eq!(
        aead::open_committing(&recovery_key, &blob, &aad).unwrap(),
        vault_key
    );
}
